...
- **`@form-validator-js/validators`** — six built-in validators: `required` (text-like, radio/checkbox group, or `type="file"` for "file selected"), `minLength` / `maxLength` (code-point counted, not UTF-16 units), `pattern` (regex source), `equalsTo` (cross-field equality by id), `checkedCount` (group min/max). `type="file"` is only supported by `required` — the other validators throw `'Unsupported element type'` for it because the browser's fake-path `value` would mislead. Declares `core` as a `peerDependency` pinned to the exact same version — bump both packages together.
